...
-- Base type signature (without sum types) to avoid infinite recursion
baseTypeSignature :: Parser Token TypeSignature
baseTypeSignature =
  oneof
    [ -- Literal types (most specific, check first) TODO: allow OR between literals also does let x= 6 default to int or literal
      [TLiteralInt n | n <- satisfy $ \case Num x -> Just x; _ -> Nothing],
      [TLiteralString s | s <- satisfy $ \case StringLiteralLexed x -> Just x; _ -> Nothing],
      [TLiteralBool True | _ <- keyword "true"],
      [TLiteralBool False | _ <- keyword "false"],
      -- Base types
      [TUnknown | _ <- keyword "auto"],
      [TInt | _ <- keyword "int"],
      [TBool | _ <- keyword "bool"],
      [TString | _ <- keyword "string"],
      [TString | _ <- keyword "str"], -- TODO: change str in funlexer to string
...
typeSignature = do
  firstType <- baseTypeSignature
  restTypes <- rpt $ do
    _ <- symbol "|"
    baseTypeSignature
  case restTypes of
    [] -> return firstType
    _ -> return $ TSum (firstType : restTypes)
...
-- For function parameters: use Poly for unannotated parameters
funParam :: Parser Token TypedName
funParam = do
  name <- ident
  tSig <- optTypeSignature
  return (name, if tSig == TUnknown then Poly name else tSig)
...
funDef =
  ...
  [ Let (OnlyStr (name, TFun (snd <$> params) (fromMaybe TUnknown retType))) (Fun params body)
    | _ <- keyword "fun",
      name <- ident,
      _ <- symbol "(",
      params <- rptDropSep funParam (symbol ","),
      _ <- symbol ")",
      retType <- opt $ symbol "->" >> typeSignature,
      body <- term
  ]
    <|>
    ...
    [ Fun params body
      | _ <- keyword "fun",
        ...
        params <- rptDropSep funParam (symbol ","),
        _ <- symbol ")",
        _ <- opt $ symbol "->" >> typeSignature,  -- parse but ignore return type for anonymous functions
        body <- term
    ]
