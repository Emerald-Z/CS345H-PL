...
evalExp :: Expression -> SM.State Env Integer --given an expression, return a computation that takes an env as input and returns an integer

evalExp (Literal n) =
    return n
    
-- monads allow clean sequencing of computations, you don't have to manually pass the env around
...
evalExp Read  = do
    ...
    case input env of
        [] ->
            error "No more input"
        (i:is) -> do -- list pattern where i is the first element and is is the rest of the list
            SM.put env { input = is }
            return i
...
evalStatement (Assignment name rhs) = do
    ...
    -- lambda function for SM.modify \starts the lambda, -> separates param from body
