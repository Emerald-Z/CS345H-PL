...
{
  if j == 0 then 0
  else if j == 1 then bestEndingAt(s, 1)
  else max(bestWithinPrefix(s, j - 1), bestEndingAt(s, j))
}

function {:induction false} kadane(s: seq<int>) : (ans : int) 
{
  bestWithinPrefix(s, |s|)
}


method {:induction false} KadaneTest(s: seq<int>)
  requires |s| > 0 
{
  assert forall i :: forall j :: 0 <= i < j <= |s| ==> kadane(s) >= sum(s, i, j) by {
    KadaneProof(s);
  }

  assert exists i :: exists j :: 0 <= i < j <= |s| && kadane(s) == sum(s, i, j) by {
    KadaneProof(s);
  }
}
